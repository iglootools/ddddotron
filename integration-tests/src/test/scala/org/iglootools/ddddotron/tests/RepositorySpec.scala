...
class RepositorySpec extends Spec with SpringSupport with TableDrivenPropertyChecks with ShouldMatchers with BeforeAndAfterEach {
  ...
  val databases = Table(
    ...
    ("HSQLDB", getBean[DataSource]("hsqldbDataSource"), getBean[PlatformTransactionManager]("hsqldbTxManager")),
    ("PostgreSQL", getBean[DataSource]("postgresDataSource"), getBean[PlatformTransactionManager]("postgresTxManager"))
  )

  var currentDataSource: Option[DataSource] = None
  var currentTx: Option[TransactionStatus] = None
  var currentTxManager: Option[PlatformTransactionManager] = None
  var currentEventStore: Option[EventStore] = None
  ...
  forAll (databases) { (name:String, dataSource:DataSource, txManager:PlatformTransactionManager) =>

    implicit val eventStore: EventStore = new JdbcEventStore()(eventSerializer, aggregateRootStateSerializer, serializedEventUpgradeManager, dataSource)

    implicit val someAggregateRootFactory = SomeAggregateRoot
    val repository = SomeAggregateRootRepository()

    currentDataSource = Some(dataSource)
    currentTxManager = Some(txManager)
    currentEventStore = Some(eventStore)
